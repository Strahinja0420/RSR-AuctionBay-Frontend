            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="p-4 border-b border-gray-100 flex items-center justify-between gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <button 
                    onClick={() => { setStatusFilter("Not-Active"); setCurrentPage(1); }}
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {paginatedCategories.map((category) => {
                      const auctionCount = auctions.filter((a: any) => Number(a.categoryId) === category.id).length;
                      return (
                        <tr key={category.id} className="hover:bg-gray-50/50 transition-colors">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-center">
                            <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide ${category.isActive ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
              ...
              <div className="p-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                <p className="text-xs text-gray-500">
                  Showing <span className="font-medium">{paginatedCategories.length}</span> of <span className="font-medium">{filteredCategories.length}</span> categories
                </p>
